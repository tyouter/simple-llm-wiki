...
def _load_all_pages(config: WikiConfig) -> dict:
    pages = {}
    for subdir in ["concepts", "entities", "sources", "answers"]:
        d = config.wiki_dir / subdir
        if d.exists():
...
def _find_best_match(dead_link: str, all_titles: set) -> str | None:
...
def _add_link_to_page(info: dict, link_title: str) -> bool:
...
def _find_related_pages(title: str, content: str, pages: dict) -> list[str]:
...
def _generate_expansion(title: str, page_type: str, existing_content: str) -> str:
...
def fix_dead_links(pages: dict) -> tuple[int, int]:
        ...
        for link_target in info["links"]:
            if link_target and link_target not in all_titles:
                if link_target not in dead_to_fix:
                    best = _find_best_match(link_target, all_titles)
...
def fix_missing_bidirectional(pages: dict) -> int:
    fixed = 0
    ...
    for title, info in pages.items():
        ...
        for link_target in info["links"]:
            ...
            if _add_link_to_page(target_info, title):
                fixed += 1

    return fixed


def fix_orphan_pages(pages: dict) -> int:
    for title, info in pages.items():
        for link_target in info["links"]:
    ...
    for title, info in orphans:
        content = info["content"]
        best_links = _find_related_pages(title, content, pages)
    ...
    return fixed, len(orphans)


def fix_shallow_pages(pages: dict) -> int:
    expanded = 0
    ...
    for title, info in pages.items():
        ...
        expansion = _generate_expansion(title, page_type, content_text.strip())
...
def fix_missing_tags(pages: dict) -> int:
    fixed = 0
...
def fix_wiki(config: WikiConfig) -> dict:
    pages = _load_all_pages(config)
    total_pages = len(pages)

    fixed_dead, removed_dead = fix_dead_links(pages)
    pages = _load_all_pages(config)

    fixed_bidirectional = fix_missing_bidirectional(pages)
    pages = _load_all_pages(config)

    fixed_orphans, total_orphans = fix_orphan_pages(pages)
    pages = _load_all_pages(config)

    expanded_shallow = fix_shallow_pages(pages)
    pages = _load_all_pages(config)

    fixed_tags = fix_missing_tags(pages)

    return {
        "total_pages": total_pages,
        "dead_links_fixed": fixed_dead,
        "dead_links_removed": removed_dead,
        "bidirectional_fixed": fixed_bidirectional,
        "orphans_found": total_orphans,
        "orphans_fixed": fixed_orphans,
        "shallow_expanded": expanded_shallow,
