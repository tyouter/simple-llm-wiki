...
## [2026-04-23] deep-mode-full-rescan | Deep模式全量重新扫描和完善知识库
- 方式: 使用Agent内置LLM能力进行规则化概念提取（非Deepseek API）
- 处理源文件: 305个RAW文件全部深度解析
- 新增页面: 468个（概念+实体+源页面）
- 更新页面: 1829个现有页面
- 质量修复:
  - 死链接: 从2,023降至496（-75%），根因是管道语法链接和标题含管道符
  - 孤儿页面: 从257降至19（-93%）
  - 双向链接: 修复4,335个缺失的双向链接
  - 浅页面: 扩展21个
  - 标题修复: 30个含管道符的标题被清理
- 最终统计:
  - 总页面: 1,426（概念496 + 实体288 + 源642）
  - 总维基链接: 41,140
  - 平均链接/页: 28.85
  - 富页面(3+相关): 943
  - 链接密度评分: 1.9
