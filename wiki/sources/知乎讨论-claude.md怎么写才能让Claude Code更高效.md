...
### 全面配置方法（白玉京）
主要的详细回答提供了 Claude Code [[持久记忆系统 (Claude Code)]] 的结构化指南，主张：
- **详细的 CLAUDE.md 文件**：建议文件可长达200行，覆盖项目规则、架构、编码标准和个人偏好。
- **权限模式优化**：解释如何配置 [[权限模式 (Claude Code)]]（default、acceptEdits、plan、auto、bypassPermissions）以平衡安全性和工作流效率。
...
- **工作流优于配置**：将重心从内部 Claude 配置转移到外部开发方法论，如 [[规范编码 vs. 意念编码|规范编码]] 和 [[Humanize 工作流]]。
...
- **[[gstack-role-based-ai-development-workflow-for-claude-code]]**：虽然两者都讨论高效的 Claude Code 工作流，但这篇知乎帖子呈现了替代的、通常更简单的配置，强调需求驱动规划而非复杂的角色编排。
- **[[意念编码]]**：来源明确警告反对这种方法，主张其概念对立面 [[规范编码 vs. 意念编码|规范编码]]。
- **[[AI工具链组合]]**：提到在 [[Humanize 工作流]] 中将 Claude 与 Codex 配合用于审查，是开发任务工具链组合的实践案例。
- **[[知乎讨论：AI Agent 工程师职业路径与技术方法]]**：此来源共享类似格式（详细知乎技术讨论）和平台，聚焦于 AI 辅助开发的实践实现而非职业理论。
